...
const stats = [
  { label: "Total pigeons", value: pigeons.length, change: "+3 this month", icon: Bird, tint: "bg-primary/10 text-primary" },
  { label: "Active racers", value: pigeons.filter((p) => p.status === "racer").length, change: "+1 this week", icon: Trophy, tint: "bg-accent/10 text-accent" },
  { label: "Lofts", value: new Set(pigeons.map((p) => p.loft)).size, change: "Stable", icon: Home, tint: "bg-warning/10 text-warning" },
  { label: "Total wins", value: pigeons.reduce((s, p) => s + (p.wins ?? 0), 0), change: "+8 this season", icon: TrendingUp, tint: "bg-primary-glow/10 text-primary-glow" },
];

export default function Dashboard() {
  const top = [...pigeons].sort((a, b) => (b.wins ?? 0) - (a.wins ?? 0)).slice(0, 5);
  const recent = pigeons.slice(-4).reverse();
